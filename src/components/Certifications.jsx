...
const colorMap = {
  cyan: { border: 'border-cyan-500/20', bg: 'bg-cyan-500/5', text: 'text-cyan-400', hover: 'hover:border-cyan-500/40 hover:shadow-cyan-500/10', via: 'via-cyan-500/30' },
  violet: { border: 'border-violet-500/20', bg: 'bg-violet-500/5', text: 'text-violet-400', hover: 'hover:border-violet-500/40 hover:shadow-violet-500/10', via: 'via-violet-500/30' },
  pink: { border: 'border-pink-500/20', bg: 'bg-pink-500/5', text: 'text-pink-400', hover: 'hover:border-pink-500/40 hover:shadow-pink-500/10', via: 'via-pink-500/30' },
};
...
export default function Certifications() {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.cert-card-anim',
        ...
        { y: 0, opacity: 1, scale: 1, duration: 0.7, ease: 'power3.out', stagger: 0.08,
          scrollTrigger: { trigger: '.cert-grid-container', start: 'top 85%' } }
      );
  ...
  return (
    <section ref={sectionRef} id="certifications" className="py-24 px-6 lg:px-8 bg-[#030712] relative overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-violet-600 rounded-full filter blur-[200px] opacity-5 pointer-events-none"></div>
      <div className="absolute inset-0 grid-pattern pointer-events-none"></div>

      <div className="max-w-7xl mx-auto relative z-10">
        
        {/* Header */}
        <div className="text-center mb-16 cert-card-anim">
        ...
        {/* Cards Grid */}
        <div className="cert-grid-container grid md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {certifications.map((cert, i) => {
            const c = colorMap[cert.color];
            return (
              <div 
                key={i} 
                className={`cert-card-anim tilt-card-cert group p-5 rounded-2xl border ${c.border} ${c.bg} bg-[#0a0f1e] ${c.hover} hover:shadow-xl transition-colors duration-300 cursor-default`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-neutral-400 text-xs mb-3">{cert.date}</p>
                ...
                {/* Fixed Dynamic Class warning using colorMap configuration */}
                <div className={`mt-4 h-px bg-gradient-to-r from-transparent ${c.via} to-transparent transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500`}></div>
              </div>
            );
          })}
        </div>
